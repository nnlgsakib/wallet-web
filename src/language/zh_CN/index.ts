export default {
  ...
  validator: {
    ...
    pledgeAmountErr: "质押金额不能小于350ERB",
    pledgeAmountErr1: "质押金额必须大于0",
    redemAmountErr: "赎回金额不能大于已质押的金额",
    redemAmountErr2: "赎回金额不能为空",
    redemAmountErr3: "赎回金额必须大于0",
    fromPledgeErr: "当前要质押的地址质押金额必须要大于350",
    updateLoadingText: "数据更新中...",
    leftPledgeErr: "最少质押金额为350ERB，您可以选择赎回全部质押金额",
    totalStaking: "总质押金额",
    amount: "账户余额",
    amountTip: "提示:您帐户中的剩余可用余额。",
    ownValidator: "您自己的验证器地址",
    ownValidatorTip:
      "验证者需要将至少350个ERB押注到他们自己的地址，才能成为合格的完整节点，他们押注的越多，他们的权重就越高。",
    otherValidator: "其他验证器地址",
    otherValidatorTip:
      "通过将ERB与具有高在线权重的验证器或指定的验证器进行押注，参与者可以参与SNFT挖矿。",
  ...
  wallet: {
    ...
    conver_waiting:
      "{count}<span style='color:#9F54BA;'>个S-NFT{countstr}</span>正在兑换，兑换成功，将获得{amount}<span style='color:#9F54BA;'>ERB</span>，您也可以在个人中心的历史记录查看兑换详情",
    conver_success:
      "{count}<span style='color:#9F54BA;'>个S-NFT</span>兑换成功，已获得{amount}<span style='color:#9F54BA;'>ERB</span>，您也可以在个人中心的历史记录查看兑换详情",
    conver_wrong:
      "{count}个S-NFT兑换失败，您可以在个人中心历史记录查看兑换详情",
  },
  converSnft: {
    converTit: "转换为 ERB",
    select: "选择",
  ...
  common: {
    ...
    exchange_pledge:
      "您需要质押至少350ERB来开启您的质押者,在成功开启您的质押者后，您将有机会获得区块链的出块补贴，您的质押金额越高，被选中的概率也越高。",
    no_data: "无数据",
    mainNetwork: "主网",
    save: "保存",
    closed: "关闭",
    ...
    moveDataTit: "提示",
    moveTip:
      "因版本更新导致需要手动恢复钱包数据，确认恢复旧版本的钱包数据到该钱包吗？",
    moveTip2: "正在迁移数据，请稍等！",
    settingTip: "设置更多",
    switchNet: "切换网络",
    helpCenter: "帮助中心",
    ...
    confirmExit: "确认退出",
    guideBtnTip:
      "您的数字钱包数据仅保存在您浏览器的本地存储中；请务必创建备份以避免潜在的永久性损失。卸载浏览器和/或清除浏览器数据也可能导致永久性损失",
    asyncData: "数据同步中，请稍后",
    convertAmount: "转换金额",
    loadingWait: "加载中，请稍后!",
    ...
    snftColorTip:
      "S-NFT等级从低到高分别为L0、L1、L2、L3。您可以合成更高等级以获取更高收益，合成规则如下：16个特定S-NFT L0可合成一个唯一S-NFT L1，16个特定S-NFT L1可合成一个唯一S-NFT L2，16个特定S-NFT L2可合成一个唯一S-NFT L3。蓝色数字表示在某一个S-NFT L1中，SNFT L0的位置编号。绿色数字表示在某一个S-NFT L2中，SNFT L1的位置编号。黄色数字表示在某一个S-NFT L3中，SNFT L2的位置编号。红色数字表示S-NFT L3的位置编号。",
    contrctTx: "合约交易",
    contractAddr: "合约地址",
    converTip:
      "提示: S-NFT交换成功后，将从钱包中消失。您可以在钱包的交易历史记录或浏览器中查看交易记录。",
    snftLabel: "S-NFTs转换",
    hsitory: "交易历史",
  ...
  generateNFT: {
    ...
    promptTip1:
      "使用人工智能:输入你想要描绘的提示或描述，选择人工智能模式，并支付一定的ERB费用。AI将专门为您创建NFT内容并将其发送到您的电子邮件。",
    promptTip2:
      "使用URL:输入现有NFT内容的URL，这些内容可以从OpenSea等可以读取图像信息的平台上获取。",
    promptTip: "可以通过两种方式生成链层NFT:",
    placeholder:
      "提示词:请输入文字描述图片，例如:按照维米尔的《戴珍珠耳环的女孩》的风格画一个海唐。 \nSourceUrl:资产URL(长度不超过112)",
    promptWordErr: "最多可输入70个字符串，只支持英文俚语、特殊符号或url地址",
    promptWordIsSensitive:
      '您的提示词涉及到敏感词汇，请修改后再提交，"{words}"',
    promptWordNotNull: "提示词/SourceUrl不能为空",
    normalNftTip:
      "您已勾选AI绘画，请填写有效提示词，最多输入70个字符串，仅支持英语俚语和特殊符号。",
    creativeMode: "AI绘画模式",
    AIDrawing: "开启AI绘图",
    AIDrawing2: "AI绘图",
    emailAddr: "邮箱地址",
    ...
    drawVal: "AI绘画费用",
    aiDrawTip:
      "输入AI提示词并选择此模式后，AI会为您创建一个NFT。这需要少量的ERB费用，在测试网阶段，{money} ERB是测试标准。",
  ...
  send: {
    ...
    failMessage:
      "非常抱歉，因为某些意外，操作失败，您可以去交易历史里查看交易的具体信息",
  },
  // 交易详情
  transactiondetails: {
    status: "状态",
  ...
  sendto: {
    ...
    nomoney:
      "您的账户有{amount}{symbol}可用支付交易费。购买一些{symbol}或从其他账户存入。",
    erroraddress: "地址格式有误",
    buy: "购买ETH",
    next: "下一步",
    from: "从",
  ...
  createExchange: {
    ...
    create_approve:
      "正在发送成为质押者的请求，您也可以通过交易历史查看请求的详情",
    ...
    pageTip:
      "这是质押者管理页面。如果你想去质押者的页面，点击“质押者”。如果你想进入质押者的控制台，点击“控制台”。如果你想添加质押量或赎回你的质押，点击在“Staking & Redemption”。如果要更改服务器的设置，请单击“Server”。",
    serveTip1: "这是您必须为服务器支付一年的费用。",
    serveTip2: "这是您离开我们服务器的使用时间。",
    serveTip3: "这是您为本次交易购买的我们服务器的使用时间",
    addStaking:
      "该交易将帮助您将额外的ERB存入您的账户。您获得奖励的机会将增加。您的额外ERB也可以在几个月后赎回。",
    redeem: "这是您要赎回的 ERB 数量",
    convert: "转换",
    insufficientMoney:
      "在质押后，您的余额将低于201ERB，这不足以在我们的服务器上建立您的质押者。你需要支付额外的{value}ERB。",
  },
  // 关闭质押者
  closeexchange: {
    closeexchange: "关闭质押者",
  ...
  minerspledge: {
    ...
    resetBtnTip:
      "有了这个按钮，你可以恢复你的信誉到70信誉将花费 {value} ERB。ERB成本将被Erbie区块链烧毁。",
    noMoney: "请保证账户余额大于{value}ERB以成功恢复权重！",
    resetSendTip:
      "通过信誉恢复，可以最大程度恢复您的信誉。 一个信誉将花费一个 ERB。",
    ...
    close_approve:
      "正在通过解冻质押的请求，您也可以通过交易历 史查看请求的详情",
    close_waiting: "正在为您解冻质押，此过程将可能花费十几秒",
    close_success:
      "已成功为您解冻质押，质押金额已经退回您的账户中，您也可以通过交易历史查看交易的详情",
    close_wrong:
      "非常抱歉，因为某些意外，操作失败，您可以去交易历史里查看交易的具体信息",
    toggleSuccess: "切换成功",
    node: "节点(非必填)",
    ...
    minus_approve:
      "我们正在批准您的股份赎回请求。您可能需要等待几秒钟。您也可以在交易记录中找到详细信息。",
    minus_waiting: "我们正在进行股权赎回。请稍等。",
    minus_success: "您的股份赎回已完成。您可以在交易记录中找到详细信息。",
    minus_wrong:
      "非常抱歉，由于一些意外，操作失败，您可以到交易记录查看交易的具体信息",
  },
  // 创建矿工节点
  createminerspledge: {
    openexchange: "开启质押者",
  ...
  bourse: {
    ...
    custom: "设置",
    tip13:
      "如果您不使用我们的服务，在质押您的 ERB 以打开 NFT 市场后，您需要按照 wiki 说明在您自己的服务器上设置您的 NFT 市场。",
    ...
    periodAdd: "服务器时间追加",
    addServeAmount:
      "您可以租用我们的服务器进行质押者，每年需要支付200ERB。支付后，您就可以在我们的服务器上设置您的质押者。",
    minusStaking: "在您撤销350 ERB质押后，您将没有机会再次获得SNFT奖励。",
